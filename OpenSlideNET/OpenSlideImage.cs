...
public sealed class OpenSlideImage : ISlideImage
{
    ...
    [MemberNotNull(nameof(dimensionsCache))]
    ...
    public long? BoundsX
    {
        get
        {
            if (TryGetProperty(OpenSlideInterop.OpenSlidePropertyNameBoundsX, out var value) && long.TryParse(value, out var result))
            {
                return result;
            }
            return null;
    ...
    public long? BoundsY
    {
        get
        {
            if (TryGetProperty(OpenSlideInterop.OpenSlidePropertyNameBoundsY, out var value) && long.TryParse(value, out var result))
            {
                return result;
            }
            return null;
    ...
    public long? BoundsWidth
    {
        get
        {
            if (TryGetProperty(OpenSlideInterop.OpenSlidePropertyNameBoundsWidth, out var value) && long.TryParse(value, out var result))
            {
                return result;
            }
            return null;
    ...
    public long? BoundsHeight
    {
        get
        {
            if (TryGetProperty(OpenSlideInterop.OpenSlidePropertyNameBoundsHeight, out var value) && long.TryParse(value, out var result))
            {
                return result;
            }
            return null;
    ...
    private double? GetMicronsPerPixel(char dimension)
    {
        ...
        if (TryGetProperty("tiff.ResolutionUnit", out var u))
        {
            ...
            if (unit > 0 &&
                TryGetProperty($"tiff.{dimension}Resolution", out var resolution) &&
    ...
    {
        if (!TryGetProperty($"openslide.level[{level}].tile-width", out var w) || !int.TryParse(w, out var width))
        {
            width = 0;
        }

        if (!TryGetProperty($"openslide.level[{level}].tile-height", out var h) || !int.TryParse(h, out var height))
        {
            height = 0;
        }
